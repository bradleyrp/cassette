...
"""
issues: 
- cannot end in a list or references gets screwed up
"""
...
class MDHeaderText:
	...
	def __init__(self,lines):
		...
		# extract the header
		#! very hard to figure out  the \s? below! actually no it was \r from david instead of \n and I realized it later on failure to find an image: missing_images ['ppi_chemistry.png\r']
		regex_header_block = r"-{3,}\s?\n(.*?\n-{3,})(.+)"
		header_pull = re.compile(regex_header_block,flags=re.M+re.DOTALL)
...
class TexDocument:
	...
	spacing_chars = '\s:\.,\)'
	#! previously: bibkey = '[a-zA-Z\-]+-[0-9]{4}[a-z]?'
	#! get the alternate bibkey from the header?
	bibkey = '\{?[a-zA-Z\-]+[-:]?[0-9]{4}[a-z]{0,3}\}?'
	...
	rules_tex = {
		...
		#! put the label right after the section instead of inside braces for jacs template # nope reversed
	...
	subs_tex = odict([
		...
		('\<\<\<(.*?)\>\>\>',r""), # hide some comments
		('\[([^\]]+)\]\(([^\)]+)\)(?:{:target="_blank"})?',r'\href{\2}{\1}'),
		('\<\<([^\>]+)\>\>',
			# only works with jacs:
			r"\hlcyan{\1}"),
	...
	subs_html = odict([
		...
		('\*\*([^\*]+)\*\*',r'<strong>\1</strong>'),
		('\*([^\*]+)\*',r'<em>\1</em>'),
		('\[\[([^\]]+)\]\]',r"""<span style="background-color: #ffa">\1</span>"""),
		('\<\<\<(.*?)\>\>\>',r""), # hide some comments
		...
		('(?:``)([^\']+)(?:\'\')',r"&#8220;\1&#8221;"),
		('`([^`]+)`',r"<code>\1</code>"),
		('\[([^\]]+)\]\(([^\)]+)\){:target="_blank"}',r'<a href="\2" target="_blank">\1</a>'),
		('\[([^\]]+)\]\(([^\)]+)\)',r'<a href="\2">\1</a>'),
		...
		(r"\\vspace\{([^\}]+)\}",""),
		#! removing this for the nonbreaking space (r"~(.*?)\|(.*?)~",r"\2"),
		# latex-only refs get some styling that harkens to tex
		(r"\\ref{(.*?)}",r"@{\1}"),
		(r"\|\|(.*?)\|\|",r"<del>\1</del>"),]) #! tilde is usually nonbreaking space in latex
	...
	def __init__(self,fn,**kwargs):
		...
		# enclose refs in braces or error when inside highlights
		...
		self.subs_tex.update(**{'@sec:(%s+)'%self.labelchars:
			#! fixed for jacs lambda x:self.secstyle%(r"{\ref{sec:%s}}"%(underscore(x.group(1))))})
			lambda x:self.secstyle%(r"{\hyperref[sec:%s]{%s}}"%(underscore(x.group(1)),underscore(x.group(1))))})			
		...
		#! ^^ wut? something to do with makefile?
	...
	def bibliography_html(self):
		...
		reforder_non_unique = [re.match('^{?(.*?)\}?$',i).group(1) for i in 
			#re.findall(regex_bibref%self.bibkey,self.body)]
			# need to obey subs first
			re.findall(regex_bibref%self.bibkey,'\n'.join(self.parts['body']))]
		reforder = []
		...
		for key in sorted(ordlookup.keys()):
			...
			#! authors can have braces with special characters followed by commas so we use newline below
			authors = ''.join(re.findall('(?:A|a)uthor\s*=\s*\{(.*?)\},\n',dat))
			try: 
				year = int(re.findall('(?:Y|y)ear\s*=\s*\{([^\}]+)',dat)[0])
			except: 
				raise Exception('[ERROR] cannot find bibkey "%s" in the database'%key)
			title = re.sub('([\{\}]+|\{.+\})','',re.findall('(?:T|t)itle\s*=\s*\{(.*?)},',dat)[0])
				...
				for found in re.findall('@(%s)+'%self.bibkey,line):
					...
					if 1:#try:
						...
						if False:
							...
							chunk = ('<span class="tooltip">[<a href="#refno%(refno)s">%(refno)s</a>'+
								'<span class="tooltiptext">'+
								'<a href="#refno%(refno)s">'+
								'<code>%(bibkey)s(%(year)s)</code><br>%(title)s</a></span>]</span>')%{
								'refno':ordlookup[found_strip],'title':details_raw[found_strip]['title'],
								'bibkey':re.match('^(.*?)[\:\-\d]',found_strip).group(1),
								'year':details_raw[found_strip]['year']}
							self.parts['body'][lineno] = re.sub('@%s'%found,chunk,self.parts['body'][lineno])
					#except: raise Exception('cannot link %s'%found)

		#! hacking substitutions for latex characters here. didn't work in the subs_html for main text
		subs = [(r'{\\"u}','u'),(r'{\\textquoteright}','\''),(r'{\\"o}','o'),(r"{\\'e}",'e'),(r'{\\"e}','e'),
			(r"{\\'o}",'o'),(r"{\\'a}",'a')]

		# loop over references at the end
		for refno,ref in enumerate(reforder):
			refkey = bibkeys.index(ref)
			det = str(details[bibkeys[refkey]])
			for i,j in subs: det = re.sub(i,j,det,re.M+re.DOTALL)
			html.append('<li id="refno%d"><a name="refno%d"></a>%s</li>'%(refno+1,
				refno+1,det))
	...
	def proc(self,part='body',version='latex'):
		...
		if any(missing_images):
			raise Exception('[ERROR] missing images:\n%s\n'%'\n'.join(missing_images))
	...
	def write_html(self,fn,dn):
		...
		self.parts_list.append('closer')
		self.parts['closer'] = """</div></div></div></body>"""
	...
	def figure_convert_tex(self,extracts):
		...
		#! hacking for JACS using exclamation and \usepackage[section]{placeins} because the figures are all jacked up near the supplement sections and the references!
		#! also hacking for jacks with max height below
		text = (r"\begin{figure}[H]"+'\n'+r"\centering"+'\n'+
			r"\includegraphics[width=%.2f\linewidth,max height=1.0\linewidth]{%s}"%(width,path)+
	...
	def bib(self,part='body'):
		...
		"""
		#! missing refs and got acemso? then you'll get the following error:
		#! ... "! Undefined control sequence.\@mciteMaxWidthAuxWrite ...correctmaxwidthsubitem"
		# use re.split and re.findall to iteratively replace references in groups
		for lineno,line in enumerate(self.parts[part]):
			#! updated for new citation method
			if re.search('(\[?@\{?[a-zA-Z]+[-:]?[0-9]{4}[a-z]{0,3}\}?\s?;?\s?)+\]?',line)!=None:
				...
				for ii,i in enumerate(notrefs[1:]):
					if inside_reference == False: 
						# extra braces in case inside of hl
